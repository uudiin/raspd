...
int wide_band; /* 2Mbs data rate in use? */
int p_variant; /* 0 for RF24L01 and 1 for RF24L01P */
...
void powerUp(void)
{
	int up = read_register(CONFIG) & _BV(PWR_UP);
...
void startFastWrite( const void* buf, uint8_t len, const int multicast){ /* TMRh20 */
...
void maskIRQ(int tx, int fail, int rx){
