...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "p:f:su", options, NULL)) != -1) {
        switch (c) {
            ...
            default:
                      MPL_LOGE("usage: %s --pin-int <n> [hostname] [port]\n", argv[0]);
                      exit(1);
        }
    }
    ...
    if (self_test) {
        ...
        if (result == 0) {
            MPL_LOGE("Passed!\n");
            MPL_LOGE("accel: %7.4f %7.4f %7.4f\n",
                    accel[0]/65536.f,
                    accel[1]/65536.f,
                    accel[2]/65536.f);
            MPL_LOGE("gyro: %7.4f %7.4f %7.4f\n",
                    gyro[0]/65536.f,
                    gyro[1]/65536.f,
                    gyro[2]/65536.f);
    ...
    if (hostname && long_port) {
        eMPL_connect(hostname, long_port, use_udp);
    }
