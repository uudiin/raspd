...
unsigned char dev_addr = MPU6050_DEFAULT_ADDRESS;
...
static int read_regs(unsigned char reg, unsigned char *buffer, size_t size)
{
    int err;
 ...
 */
int i2c_writeBits(uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t data) {
    //      010 value to write
 ...
 */
int i2c_readBits(uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t *data) {
    // 01101001 read byte
    // 76543210 bit numbers
    //    xxx   args: bitStart=4, length=3
    //    010   masked
 ...
 */
static void setSleepEnabled(int enabled) {
